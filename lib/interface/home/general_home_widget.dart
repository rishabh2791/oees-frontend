...
class _GeneralHomeWidgetState extends State<GeneralHomeWidget> {
      ...
      if (response.containsKey("status") && response["status"]) {
        for (var item in response["payload"]) {
          ...
          if (shiftEndHour > shiftStartHour) {
            shiftStart = DateTime(now.year, now.month, now.day, int.parse(startTime.split(":")[0].toString()), int.parse(startTime.split(":")[1].toString()));
            shiftEnd = DateTime(now.year, now.month, now.day, int.parse(endTime.split(":")[0].toString()), int.parse(endTime.split(":")[1].toString()));
          } else {
            if (hour < 12) {
              shiftStart = DateTime(yesterday.year, yesterday.month, yesterday.day, int.parse(startTime.split(":")[0].toString()), int.parse(startTime.split(":")[1].toString()));
              shiftEnd = DateTime(now.year, now.month, now.day, int.parse(endTime.split(":")[0].toString()), int.parse(endTime.split(":")[1].toString()));
            } else {
              shiftStart = DateTime(now.year, now.month, now.day, int.parse(startTime.split(":")[0].toString()), int.parse(startTime.split(":")[1].toString()));
              shiftEnd = DateTime(tomorrow.year, tomorrow.month, tomorrow.day, int.parse(endTime.split(":")[0].toString()), int.parse(endTime.split(":")[1].toString()));
            }
          }
        ...
        for (var item in response["payload"]) {
          ...
          if (!lineIP.containsKey(line.id)) {
            if (line.ipAddress != "" || line.ipAddress.isNotEmpty) {
              lineIP[line.id] = "ws://" + line.ipAddress.replaceAll(" ", "") + ":8001/";
            }
          }
        }
      }
  ...
  Future<void> getHours() async {
    ...
    int hour = 1;
    while (hourStart.difference(shiftEndTime).inSeconds < 0 && hourStart.difference(DateTime.now()).inSeconds < 0) {
      shiftHours[hour] = {
  ...
  Future<void> getRunningTaskBatches() async {
    for (var key in tasksByLine.entries) {
      bool runningTasks = tasksByLine[key.key]!.any((task) => task.endTime.toLocal().difference(DateTime.parse("2099-12-31T23:59:59Z").toLocal()).inSeconds == 0);
      if (runningTasks) {
    ...
    for (var lineID in lineIDs) {
      ...
      shiftHours.forEach((key, value) {
        if (DateTime.now().difference(DateTime.parse(value["start_time"]!)).inSeconds > 0) {
          ...
          double linePeriodControlledDowntime = getTotalDowntime(DateTime.parse(value["start_time"]!), DateTime.parse(value["end_time"]!), lineID, "Controlled").toDouble();
          double linePeriodPlannedDowntime = getTotalDowntime(DateTime.parse(value["start_time"]!), DateTime.parse(value["end_time"]!), lineID, "Planned").toDouble();
          double linePeriodUnplannedDowntime = getTotalDowntime(DateTime.parse(value["start_time"]!), DateTime.parse(value["end_time"]!), lineID, "Unplanned").toDouble();
          double linePeriodProduction = getTheoreticalTotalProduction(DateTime.parse(value["start_time"]!), DateTime.parse(value["end_time"]!), lineID);
          double actualPeriodProduction = getActualTotalDeviceData(DateTime.parse(value["start_time"]!), DateTime.parse(value["end_time"]!), lineID, true);
          ...
          lineTotalTime =
              lineTotalTime + (DateTime.parse(value["end_time"]!).difference(DateTime.now()).inSeconds > 0 ? DateTime.now().difference(DateTime.parse(value["start_time"]!)).inSeconds : 3600);
  ...
  Widget getOtherDeviceData() {
    double minWeight = getRunningTaks(selectedLine.text) is String ? 0 : getRunningTaks(selectedLine.text).minWeight;
          ...
          if (device.deviceType.toUpperCase() == "SCALE") {
            ...
            for (int i = 0; i < last; i++) {
              thisWidgets.add(Text(
                ...
                style: TextStyle(
                  fontSize: 30.0,
                  color: weightsByLineID[selectedLine.text]![i] < minWeight ? Colors.red : Colors.green,
                ),
              ));
            }
          } else {
            if (key.split("_")[0] == lineID) {
              ...
              for (int i = 0; i < last; i++) {
                thisWidgets.add(Text(
                  ...
                  style: TextStyle(
                    fontSize: 30.0,
                    color: value[i].value < minWeight ? Colors.red : Colors.green,
                  ),
                ));
              }
            }
    ...
    await appStore.deviceApp.list(deviceCondition).then((response) async {
      if (response.containsKey("status") && response["status"]) {
        ...
        await appStore.deviceDataApp.list(deviceDataCondition).then((value) {
          if (value.containsKey("status") && value["status"] && device.deviceType.toUpperCase() == "SCALE") {
    ...
    return [
      charts.Series<BadRateProduction, String>(
        ...
        domainFn: (BadRateProduction downtime, _) =>
            shiftHours[downtime.hour]!["start_time"]!.split(" ")[1].substring(0, 5) + "\n" + shiftHours[downtime.hour]!["end_time"]!.split(" ")[1].substring(0, 5),
                              ...
                              child: Column(
                                children: [
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        "Line: " + lines.where((element) => element.id == selectedLine.text).toString().replaceAll("(", "").replaceAll(")", ""),
                                        style: TextStyle(
                                      ...
                                      SizedBox(
                                        ...
                                        child: Text(
                                          getRunningTaks(selectedLine.text).runtimeType == String
                                              ? "SKU: "
                                              : "SKU: " + getRunningTaks(selectedLine.text).code + " - " + getRunningTaks(selectedLine.text).description,
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Text(
                                        lineAvailability.containsKey(selectedLine.text) ? "Availability: " + lineAvailability[selectedLine.text]!.toStringAsFixed(2) : "Availability: 0",
                                        style: const TextStyle(
                                          color: Colors.red,
                                          fontSize: 25.0,
                                        ),
                                      ...
                                      Text(
                                        linePerformance.containsKey(selectedLine.text) ? "Performance: " + linePerformance[selectedLine.text]!.toStringAsFixed(2) : "Performance: 0",
                                        style: const TextStyle(
                                          color: Colors.red,
                                          fontSize: 25.0,
                                        ),
                                      ...
                                      Text(
                                        lineQuality.containsKey(selectedLine.text) ? "Quality: " + lineQuality[selectedLine.text]!.toStringAsFixed(2) : "Quality: 0",
                                        style: const TextStyle(
                                          color: Colors.red,
                                          fontSize: 25.0,
                                        ),
                                  ...
                                  runningTaskBatchByLine.containsKey(selectedLine.text)
                                      ? Row(
                                          ...
                                          children: [
                                            ...
                                            Text(
                                              "Batch Size: " + runningTaskBatchByLine[selectedLine.text]!.batchSize.toStringAsFixed(1).replaceAllMapped(reg, (Match match) => '${match[1]},') + " KG",
                                              style: TextStyle(
                                                color: isDarkTheme.value ? foregroundColor : backgroundColor,
                                                fontSize: 25.0,
                                              ),
                                  ...
                                  runningTaskBatchByLine.containsKey(selectedLine.text)
                                      ? Row(
                                          ...
                                          children: [
                                            Text(
                                              "Expected Batch Units: " +
                                                  (runningTaskBatchByLine[selectedLine.text]!.batchSize *
                                                          1000 /
                                                          (getRunningTaks(selectedLine.text).runtimeType.toString() == "String" ? 1 : getRunningTaks(selectedLine.text).expectedWeight))
                                                      .toStringAsFixed(0)
                                            ...
                                            Text(
                                              "Actual Batch Units: " + (runningTaskCount.toStringAsFixed(0).replaceAllMapped(reg, (Match match) => '${match[1]},')),
                                              style: TextStyle(
